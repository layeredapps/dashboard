...
describe('/api/user/set-account-deleted', () => {
  ...
  describe('configuration', () => {
    it('environment DELETE_DELAY', async () => {
      ...
      const days = Math.floor((new Date(accountNow.deletedAt).getTime() - new Date().getTime()) / 1000 / 60 / 60 / 24)
      const days2 = Math.ceil((new Date(accountNow.deletedAt).getTime() - new Date().getTime()) / 1000 / 60 / 60 / 24)
